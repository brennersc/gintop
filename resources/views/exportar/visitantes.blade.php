...
$itens = DB::table('credenciamentos')
        ...
        ->selectRaw('GROUP_CONCAT(valor_salvo ORDER BY id ASC) as valor_salvo')
        ->selectRaw('GROUP_CONCAT(id ORDER BY id ASC) as id')
...
<table class="table table-ordered table-bordered table-hover table-striped table-sm" id="tabelaUsuario">
        <thead>
                <tr>
                        {{-- <th style="display:none"><b>Nome</b></th> --}}
                        {{-- <th><b>Email</b></th>
                        <th><b>Celular</b></th> --}}
        ...
        <tbody>
                @foreach ($itens as $tabela)
                        ...
                        @endphp
                <tr>
                        {{-- <td style="display:none">{{$tabela->nome}}</td> --}}
                        {{-- <td>{{$tabela->email}}</td>
